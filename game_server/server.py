        ...
        while self.running:
            ...
            try:
                self.producer.produce(
                    ...
                    # key=None, # Game state is global, not usually keyed by player
                )
                # Poll is important for callbacks and to ensure messages are sent.
                # A poll(0) is non-blocking. If producing rapidly, a small positive timeout might be better.
                self.producer.poll(0)
            except BufferError:
                # This can happen if the produce queue is full.
                # producer.poll(0.1) might help clear it, or indicates a deeper issue (e.g., broker down).
            ...
            # else:
            #     print(f"Server: Game loop took longer than tick rate: {elapsed_time:.4f}s") # For performance tuning
    ...
    def start(self):
        ...
        try:
            while self.running: # Keep main thread alive, check running flag
                # Check if threads are alive, or implement a more graceful shutdown signal
